...
def chat():
    if 'messages' not in flask.request.json:
        return flask.jsonify({'status': 'error', 'error': '`messages` is `null`. Please provide a list of messages in OpenAI API format.'}), 400
    if 'retrieved_stories' not in flask.request.json:
        return flask.jsonify({'status': 'error', 'error': '`retrieved_stories` is `null`. Please provide a list of stories, even if empty.'}), 400
    
    messages = flask.request.json['messages']
    retrieved_stories = flask.request.json['retrieved_stories']
    
    messages_with_prompt = [
        {"role": "system", "content": "You are a compassionate chatbot designed to help people navigate and understand challenges in their lives. You will be provided stories from other people to reference in your responses to the user's messages."}
    ]
